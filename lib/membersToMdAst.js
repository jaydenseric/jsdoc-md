  ...
  const recurse = (members, depth) => {
    members.forEach(member => {
      ...
      if (member.kind === 'typedef' || member.kind === 'function') {
        ...
        if (returnsTag) {
          const { type, description } = returnsTag

          if (!description)
            throw new Error(
              `Missing JSDoc return description for namepath “${
                member.namepath
              }”.`
            )

          const children = [
            { type: 'strong', children: [{ type: 'text', value: 'Returns:' }] }
          ]
          ...
          children.push(
            { type: 'text', value: ' ' },
            ...mdToMdAst(description, outlinedMembers)
          )
